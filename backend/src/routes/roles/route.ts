...
async function writeCategories(categories: any[]): Promise<boolean> {
  try {
...
export async function GET(req: Request) {
  ...
  try {
    ...
    if (categoryId) {
      const category = categories.find((cat: { id: string, roles?: any[] }) => cat.id === categoryId);
      if (!category) {
    ...
    const allRoles = categories.flatMap((cat: { roles?: any[] }) => cat.roles || []);
...
export async function PUT(req: Request) {
  try {
    ...
    const roleIndex = category.roles.findIndex((r: { id: string }) => r.id === role.id);
    if (roleIndex === -1) {
...
export async function DELETE(req: Request) {
  try {
    ...
    const updatedRoles = category.roles.filter((role: { id: string }) => role.id !== roleId);
