...
make_request "POST" "http://localhost:3001/api/calendar-events" '{
  ...
  "startDate": "2025-04-02T14:00:00Z",
  "endDate": "2025-04-02T15:00:00Z",
...
# Capture the ID of the first event
EVENT_ID=$(curl -s "http://localhost:3001/api/calendar-events" | jq -r '.[0].id')
echo "Captured EVENT_ID: $EVENT_ID"

# Test 4: Get calendar event by ID
echo -e "\n\nTest 4: Get calendar event by ID"
make_request "GET" "http://localhost:3001/api/calendar-events/$EVENT_ID"

# Test 5: Update calendar event
echo -e "\n\nTest 5: Update calendar event"
make_request "PUT" "http://localhost:3001/api/calendar-events/$EVENT_ID" '{
  "title": "Updated Event",
...
echo -e "\n\nTest 6: Delete calendar event"
make_request "DELETE" "http://localhost:3001/api/calendar-events/$EVENT_ID"
